...
// FiveM bundles Node.js 16 which lacks native fetch — polyfill with built-in https/http
...
async function takeScreenshot(playerId) {
  ...
  try {
    const presignResult = await new Promise((resolve, reject) => {
      ...
      fetch(ingressUrl + '/api/screenshot/presign?licenseKey=' + encodeURIComponent(licenseKey), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      }).then(async (res) => {
        clearTimeout(t);
        if (res.ok) {
          try { resolve(await res.json()); } catch (e) { reject(new Error('invalid json')); }
        } else {
          reject(new Error('presign http ' + res.status));
        }
  ...
  // Step 2a: try screenshot-basic (uploads directly from client NUI to ingress)
  ...
  if (uploaded) return publicUrl;

  // Step 2b: fallback — trigger VexonAC NUI to capture and upload to the presign URL.
  // The ingress returns a Discord-compatible response so the NUI can parse the URL,
  // which is then stored in the player's state bag as ScreenShotURL.
  try {
    const pid = parseInt(pidStr, 10);
    const stateBefore = GetStateBagValue('player:' + pid, 'ScreenShotURL') || '';
    TriggerClientEvent('__VexonAC:takeScreenShot', pid, uploadUrl);

    const deadline = Date.now() + 12000;
    while (Date.now() < deadline) {
      await new Promise(r => setTimeout(r, 400));
      const current = GetStateBagValue('player:' + pid, 'ScreenShotURL') || '';
      if (current && current !== stateBefore) return current;
    }

    // NUI didn't update state — return the pre-allocated publicUrl anyway
    // (ingress stored the file if upload succeeded)
    return publicUrl;
  } catch (e) {
    return publicUrl;
  }
