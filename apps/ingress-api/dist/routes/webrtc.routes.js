...
export function createWebRTCRouter({ io }) {
    const router = Router();
    ...
    router.post("/offer", async (req, res) => {
        try {
            ...
            console.log("📡 [WebRTC] Forwarding offer to FiveM server for player:", playerId);
            // Find the active stream to get the FiveM server URL
            const activeStream = Array.from(activeStreams.values()).find((stream) => stream.playerId === playerId);
            ...
            const fivemResponse = await fetch(`${activeStream.fivemServerUrl}/webrtc/offer`, {
                method: "POST",
                headers: {
            ...
            if (!fivemResponse.ok) {
                throw new Error(`FiveM server responded with status: ${fivemResponse.status}`);
            }
    ...
    router.post("/answer", async (req, res) => {
        try {
            ...
            console.log("📡 [WebRTC] Received answer from FiveM server for player:", playerId);
    ...
    router.post("/ice-candidate", async (req, res) => {
        try {
            ...
            console.log("🧊 [WebRTC] ICE candidate from", source || "unknown", "for player:", playerId);
    ...
    router.post("/ice-candidate-remote", async (req, res) => {
        try {
            ...
            console.log("🧊 [WebRTC] Forwarding remote ICE candidate to FiveM server for player:", playerId);
            // Find the active stream to get the FiveM server URL
            const activeStream = Array.from(activeStreams.values()).find((stream) => stream.playerId === playerId);
            if (!activeStream || !activeStream.fivemServerUrl) {
                res.status(404).json({
            ...
            const fivemResponse = await fetch(`${activeStream.fivemServerUrl}/webrtc/ice-candidate-remote`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            console.error("❌ [WebRTC] Error handling remote ICE candidate:", error);
            res.status(500).json({
                success: false,
                error: error instanceof Error ? error.message : "Internal server error",
            });
    ...
    router.post("/stream-started", async (req, res) => {
        try {
            const { playerId, streamId, playerName, serverName, startTime } = req.body;
            const fivemServerUrl = req.get("X-FiveM-Server-URL") || req.get("Origin");
            ...
            console.log("🎥 [WebRTC] Stream started:", streamId, "for player:", playerId);
    ...
    router.post("/stream-stopped", async (req, res) => {
        try {
            ...
            console.log("🛑 [WebRTC] Stream stopped for player:", playerId, reason ? `(${reason})` : "");
            // Find and remove the active stream
            const streamToRemove = Array.from(activeStreams.entries()).find(([, stream]) => stream.playerId === playerId);
            if (streamToRemove) {
    ...
    router.post("/start-stream", async (req, res) => {
        try {
            ...
            console.error("❌ [WebRTC] Error handling start stream request:", error);
            res.status(500).json({
                success: false,
                error: error instanceof Error ? error.message : "Internal server error",
            });
