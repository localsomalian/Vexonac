...
export const checkTokens = async (req, res) => {
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            // 1. Check for active bans using optimized query
            ...
            if (activeBan) {
                ...
                const realExtraIds = extraIdentifiers.filter(id => KNOWN_ID_PREFIXES.some(prefix => id.startsWith(prefix)));
        ...
        if (error instanceof z.ZodError) {
            console.error("CheckTokens ZodError:", error.errors.map((err) => ({
                field: err.path.join("."),
                message: err.message,
            })));
            res.status(400).json({
