...
export const checkPlayer = async (req, res) => {
    ...
    try {
        ...
        const realIdentifiers = identifiers.filter(id => KNOWN_ID_PREFIXES.some(prefix => id.startsWith(prefix)));
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (activeBan) {
                ...
                const bannedIdentifierData = realIdentifiers.map((identifier) => ({
            ...
            const allCurrentIdentifiersWithFid = [
                ...
                ...(Array.isArray(existingPlayer?.identifiers)
                    ? existingPlayer.identifiers.filter((id) => id.startsWith("fid:") || id.startsWith("sid:") || id.startsWith("sid2:"))
                    : []),
            ];
            if (existingPlayer) {
                ...
                const identifiersToMove = currentDbIds.filter((id) => {
                    ...
                    if (id.startsWith("discord:")) {
                        const hasAnyDiscordInNew = allCurrentIdentifiers.some((newId) => newId.startsWith("discord:"));
            ...
            const threatScoreResult = await calculateTrustScore(tx, updatedPlayer, license.id);
            const isLicenseOwner = playerDiscordIds.includes(license.discordId);
            const isBypass = isLicenseOwner || license.members.some((member) => member.permissions.includes("BYPASS") ||
                member.permissions.includes("ALL"));
            const isAdmin = isLicenseOwner || license.members.some((member) => member.permissions.length > 0);
        ...
        if (error instanceof z.ZodError) {
            ...
            console.error("CheckPlayer ZodError:", error.errors.map((err) => ({
                field: err.path.join("."),
                message: err.message,
            })));
            res.status(400).json({
