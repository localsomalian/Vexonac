...
// Type-safe file definitions with extensions
type ValidFileName =
  | "fxmanifest.lua"
  | "client.lua"
  | "auth.lua"
...
class FileService {
  private readonly filesPath = path.resolve(__dirname, "../assets/stable");
  private readonly filesPathBeta = path.resolve(__dirname, "../assets/beta");

  // Valid file names array for runtime operations
  private readonly validFileNames: ValidFileName[] = [
    "fxmanifest.lua",
    "client.lua",
    "auth.lua",
    "server.lua",
   ...
   */
  private isValidFileName(fileName: string): fileName is ValidFileName {
    return this.validFileNames.includes(fileName as ValidFileName);
  }
