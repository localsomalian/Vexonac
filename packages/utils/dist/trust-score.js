...
exports.filterIdentifiers = filterIdentifiers;
exports.findAltAccounts = findAltAccounts;
exports.findPlayerBans = findPlayerBans;
exports.findSameServerAltBans = findSameServerAltBans;
exports.findBansHistory = findBansHistory;
exports.findCrossServerBans = findCrossServerBans;
exports.calculateTrustScore = calculateTrustScore;
exports.performPlayerAnalysis = performPlayerAnalysis;
/**
...
function discordSnowflakeToDate(snowflake) {
    ...
    const snowflakeBigInt = typeof snowflake === "string" ? BigInt(snowflake) : snowflake;
 ...
 */
function filterIdentifiers(identifiers, oldIdentifiers, includeSensitive = false) {
    const current = Array.isArray(identifiers) ? identifiers : [];
    const old = Array.isArray(oldIdentifiers) ? oldIdentifiers : [];
    const all = [...new Set([...current, ...old])];
    if (includeSensitive) {
        return { all, filtered: all };
    }
    const filtered = all.filter((id) => !id.startsWith("ip:") && !id.startsWith("fid:") && !id.startsWith("sid2:"));
    return { all, filtered };
}
/**
 * Find alt accounts sharing identifiers with the player
 */
async function findAltAccounts(db, playerId, filteredIdentifiers, limit = 50) {
 ...
 */
async function findPlayerBans(db, playerId) {
 ...
 */
async function findSameServerAltBans(db, allIdentifiers, serverId, playerId) {
 ...
 */
async function findBansHistory(db, playerIds) {
 ...
 */
async function findCrossServerBans(db, filteredIdentifiers, serverId) {
    if (filteredIdentifiers.length === 0)
        return [];
 ...
 */
async function calculateTrustScore(db, player, serverId) {
    const warnings = [];
    let totalScore = 50; // Start at neutral (50)
    // Get all identifiers
    const { filtered: filteredIdentifiers, all: allIdentifiers } = filterIdentifiers(player.identifiers, player.oldIdentifiers);
    ...
    const discordIdentifier = filteredIdentifiers.find((id) => id.startsWith("discord:"));
    ...
    `);
        if (identifierFirstSeen.length > 0) {
            const daysSinceFirstSeen = Math.floor((Date.now() - identifierFirstSeen[0].firstJoin.getTime()) /
                (1000 * 60 * 60 * 24));
            if (daysSinceFirstSeen < 1) {
                ...
                warnings.push("Player identifiers are relatively new (less than 30 days)");
            }
            else {
    ...
    if (discordIdentifier) {
        try {
            ...
            if (discordId) {
                ...
                const daysSinceDiscordCreation = Math.floor((Date.now() - discordCreationDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  `);
    ...
    const daysSinceLastJoin = Math.floor((Date.now() - player.lastJoin.getTime()) / (1000 * 60 * 60 * 24));
    ...
    if (identifierAgeScore >= 10 &&
        player.playTime < 120 &&
        player.oldIdentifiers.length > 1) {
        suspiciousPatterns += 20;
        warnings.push("New account with low play time and identifier changes - possible ban evasion");
    }
 ...
 */
async function performPlayerAnalysis(db, player, contextServerId = "00000000-0000-0000-0000-000000000000", options) {
    const { showIpIdentifiers = false, showIdentifiers = true } = options;
    // Get identifier lists
    const { filtered: filteredIdentifiers, all: allIdentifiers } = filterIdentifiers(player.identifiers, player.oldIdentifiers);
    // Find alt accounts
    const altAccounts = await findAltAccounts(db, player.id, filteredIdentifiers);
    // Format alt accounts
    const formattedAltAccounts = altAccounts
        .map((alt) => {
        const { all: altAllIdentifiers } = filterIdentifiers(alt.identifiers, alt.oldIdentifiers, true // include sensitive to get full list for matching
        );
        // Find matching identifiers
        const matchingIdentifiers = altAllIdentifiers.filter((id) => filteredIdentifiers.some((originalId) => id === originalId));
        ...
        if (a.server.isCurrentServer && !b.server.isCurrentServer)
            return -1;
        if (!a.server.isCurrentServer && b.server.isCurrentServer)
            return 1;
        // If both are from same server type, sort by last join date (most recent first)
        return (new Date(b.lastJoin).getTime() - new Date(a.lastJoin).getTime());
    });
    ...
    const sameServerAltBansArray = await findSameServerAltBans(db, allIdentifiers, contextServerId, player.id);
    // Get cross-server bans using identifiers
    const crossServerBans = await findCrossServerBans(db, filteredIdentifiers, contextServerId);
    // Collect all player IDs (main + alts)
    // We cast alt.id to string to be safe, though it should be string from DB
    const relatedPlayerIds = [player.id, ...altAccounts.map((alt) => alt.id)];
    // Get bans history
    ...
    const formattedSameServerAltBans = sameServerAltBansArray.map((banData) => ({
        id: banData.ban.id,
    ...
    const formattedCrossServerBans = crossServerBans.map((banData) => ({
        id: banData.ban.id,
        banId: banData.ban.banId,
        reason: banData.ban.reason,
        details: banData.ban.details,
    ...
    const threatScore = await calculateTrustScore(db, player, contextServerId);
